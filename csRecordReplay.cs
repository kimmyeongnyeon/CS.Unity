...
public class csRecordReplay : MonoBehaviour
{
    ...
    IEnumerator coRecord()
    {
        ...
        {
            string url = "http://localhost:12000/2020_10_06/16_recordMoveRotate.php";
            string position_x = cube.position.x.ToString();// Vector X값
            string position_y = cube.position.y.ToString();// Vector Y값
            string position_z = cube.position.z.ToString();// Vector Y값
            string rotation_x = cube.rotation.x.ToString();// rotation x값
            string rotation_y = cube.rotation.y.ToString();// rotation y값
            string rotation_z = cube.rotation.z.ToString();// rotation z값
            string rotation_w = cube.rotation.w.ToString();// rotation w값
            string scale_x = cube.localScale.x.ToString(); // scale x값
            string scale_y = cube.localScale.y.ToString(); // scale y값
            string scale_z = cube.localScale.z.ToString(); // Vector X값
